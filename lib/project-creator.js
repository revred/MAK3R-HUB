...
pause`;
  ...
  generateWindowsSimpleScript(config) {
    return `@echo off
REM MAK3R-HUB Simple Launch Script
REM Quick development server start for ${config.project_name}
...
npm run dev`;
  ...
  generateWindowsDebugScript(config) {
    return `@echo off
REM MAK3R-HUB Debug Launch Script for ${config.project_name}

echo.
echo 🐛 Debug Mode: ${config.project_name}
echo 📋 Framework: ${config.framework}
...
pause`;
  ...
  generateWindowsMenuScript(config) {
    return `@echo off
REM MAK3R-HUB Menu Script for ${config.project_name}

:menu
cls
echo.
echo =========================================
echo   ${config.project_name} - MAK3R-HUB
echo =========================================
echo.
...
if "%choice%"=="1" call launch-${config.safe_project_name}.bat
if "%choice%"=="2" goto build
if "%choice%"=="3" call kill-servers.bat
...
npm run dev`;
  ...
  generateUnixSimpleScript(config) {
    return `#!/bin/bash
# MAK3R-HUB Simple Launch Script
# Quick development server start for ${config.project_name}
...
npm run dev`;
  ...
  generateUnixDebugScript(config) {
    return `#!/bin/bash
# MAK3R-HUB Debug Launch Script for ${config.project_name}
...
npm run dev -- --verbose`;
  ...
  generateUnixMenuScript(config) {
    return `#!/bin/bash
# MAK3R-HUB Menu Script for ${config.project_name}
...
menu() {
    ...
    case $choice in
        1) ./launch-${config.safe_project_name}.sh ;;
        2) build_project ;;
        3) ./kill-servers.sh ;;
        4) install_deps ;;
        5) exit 0 ;;
